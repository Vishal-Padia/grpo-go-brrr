...
# Disable HF tokenizer thread pool, this keeps the profiler trace clean.
# Must be set before any tokenizer is instantiated.
...
# Schedule: 1 wait + 1 warmup + 3 active + 1 stop. Total prof.step() calls = 5.
PROFILE_STEPS = 5
G = 8
BATCH_SIZE = 2
K = 2
...
def generate_completions(
    ...
    output = policy.generate(**gen_kwargs, past_key_values=past_key_values, use_cache=True)
    # output shape: (B*G, prompt_len + completion_len)
    # slice off the prompt to get just the completion_len
...
def build_completion_mask(completion_ids, eos_token_id):
    ...
    # argmax return 0 when there's no True, so we handle that case separately
...
def compute_logprobs(
    ...
    prompt_ids_expanded = prompt_ids.repeat_interleave(G, dim=0)  # (B*G, P)
    prompt_attn_expanded = prompt_attention_mask.repeat_interleave(G, dim=0)
...
def main():
    ...
    ref_model = torch.compile(ref_model, dynamic=True, fullgraph=False)
    ...
    # No explicit cuda.synchronize / time.perf_counter — the profiler tracks
    # CPU+GPU spans on its own. Keeping the loop minimal means the trace want to attribute time to.
    with profile(
        activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
        record_shapes=True,
        with_stack=True,
        schedule=torch.profiler.schedule(wait=1, warmup=1, active=3, repeat=1),
        on_trace_ready=torch.profiler.tensorboard_trace_handler(TRACE_DIR, worker_name="grpo_with_tc_pinned_tensors_static_kv_cache"),
    ) as prof:
        for step in range(PROFILE_STEPS):
            with record_function("rollout_setup"):
            ...
            with record_function("reward"):
                rewards, decoded = compute_rewards(
                    completion_ids, gold_answers, tokenizer, G
                )
                B = len(questions)
                rewards_grouped = rewards.view(B, G)
                advantages_grouped = (
                    rewards_grouped - rewards_grouped.mean(dim=1, keepdim=True)
                )
                advantages = advantages_grouped.view(B * G)

            with torch.no_grad():
                with record_function("logprob_old"):
            ...
            with record_function("update"):
                for inner in range(K):
                    with record_function(f"update_inner_{inner}"):
                        ...
                        loss = (
                            grpo_loss(
                                current_logprobs, old_logprobs, advantages, mask
                            )
                            + BETA * kl
                        )
