...
The real issues (the ones Dr. GRPO actually fixes) are these two:

1. Response-length normalization bias

Original GRPO token objective (per sample) is usually scaled by:

$$
\frac{1}{|o_i|}\sum_{t=1}^{|o_i|}(\cdot)
$$

This looks fair ("average per token"), but it creates bias.

When $A_i < 0$ (bad completion), longer outputs get weaker per-token penalty because of the $\frac{1}{|o_i|}$ factor.

So the model can implicitly learn: "if you're going to be wrong, be wrong at length."

That's why response length can drift upward even when quality is not improving.

2. Difficulty bias from std normalization
...
So low-variance groups (very easy or very hard prompts) can dominate updates.

But medium-difficulty prompts are usually where the most useful learning signal is.

So GRPO can overweight less-informative examples.

---

What Dr. GRPO changes

Dr. GRPO keeps the same clipped PPO-style objective, but removes these two denominators:

- remove per-response length normalization $\frac{1}{|o_i|}$
- remove std normalization $\frac{1}{\sigma}$

So advantage becomes:

$$
A_i = R_i - \mu
$$

No value model. No lambda. No correction add-on term.

Just remove the two bias-inducing normalizers.

All roads lead to ROME (ie Dr. GRPO)

We need:
- Relative comparison (good)
- Less bias from length and std normalizers (missing in original GRPO)
